...
export function buildAuthRecord(
  ...
  return {
    ...
    is_invalid_401: ex.is_invalid_401 ?? 0,
    is_quota_limited: ex.is_quota_limited ?? 0,
    is_recovered: ex.is_recovered ?? 0,
    ...
    updated_at: ex.updated_at ?? nowIso,
  };
}
...
export async function probeWhamUsage(
  ...
  const totalDeadlineMs = Math.max(5000, overallTimeoutMs ?? timeout * 1000 + 2000);

  for (let attempt = 0; attempt <= retries; attempt++) {
    const deadline = createDeadlineSignal(totalDeadlineMs);
    const perAttemptController = new AbortController();
    const perAttemptTimer = setTimeout(() => perAttemptController.abort(), timeout * 1000);
    try {
      const signal = abortSignalAny([deadline.signal, perAttemptController.signal]);
      ...
      if ((result.probe_error_kind === 'management_api_http_429' || result.probe_error_kind === 'management_api_http_5xx') && attempt < retries) {
      ...
      if (isAbort && deadline.signal.aborted) {
        result.probe_error_kind = 'timeout_total';
        result.probe_error_text = `probe overall timeout after ${totalDeadlineMs}ms`;
      } else if (isAbort) {
...
function createDeadlineSignal(ms: number): { signal: AbortSignal; cancel: () => void } {
  const controller = new AbortController();
